...
#define ao_num chbrclf_ao_num
		...
		rc =
			qmckl_set_point(context, 'N', point_num, elec_coord, point_num * 3);
		assert(rc == QMCKL_SUCCESS);
		...
		const int64_t *nucleus_shell_num =
			&(chbrclf_basis_nucleus_shell_num[0]);
		...
		rc = qmckl_get_mo_basis_mo_value(context, &(mo_value[0][0]),
										 point_num * chbrclf_mo_num);
		assert(rc == QMCKL_SUCCESS);

		double mo_vgl[point_num][5][chbrclf_mo_num];
		rc = qmckl_get_mo_basis_mo_vgl(context, &(mo_vgl[0][0][0]),
