...
#pragma omp target teams distribute parallel for collapse(2)                   \
	map(present, alloc : B, C, Updates_index)
...
#pragma omp target enter data map(alloc : workspace[0 : workspace_size],       \
...
#pragma omp target exit data map(delete : B[0 : N_updates * N_updates],        \
									 pivot[0 : N_updates], info)
...
#pragma omp target exit data map(delete : D[0 : N_updates * Lds],              \
...
#pragma omp target exit data map(delete : T1[0 : N_updates * Lds],             \
									 C[0 : Dim * N_updates])
