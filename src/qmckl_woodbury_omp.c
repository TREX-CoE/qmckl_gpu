...
qmckl_exit_code qmckl_woodbury_kxk(
  ...
  #pragma omp target teams distribute parallel for collapse(2) map(present,alloc:B,C,Updates_index)
  ...
  #pragma omp target enter data map(alloc: workspace[0:workspace_size], pivot[0:N_updates], info)
  #pragma omp target data use_device_ptr(B, workspace, pivot, info)
  {
    (void) cusolverDnDgetrf(
        s_handle,
        N_updates, N_updates,
        B, N_updates, workspace,
        pivot, &info); // col-maj enforced, so res. is LU(B)^T
  }
  ...
  {
    (void) cusolverDnDgetrs(
        s_handle,
        CUBLAS_OP_T,
        N_updates, N_updates,
        B, N_updates, pivot,
        Binv, N_updates, &info); // Needs op(B) = B^T because of line 403
  }
  #pragma omp target exit data map(delete: B[0:N_updates*N_updates], pivot[0:N_updates], info)
