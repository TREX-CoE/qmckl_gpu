...
qmckl_exit_code_device qmckl_set_point_device_from_host(qmckl_context_device context,
											  char transp, int64_t num,
											  double *coord, int64_t size_max) {
	...
	double* coord_gpu = qmckl_malloc_device(context, size_max);

	rc = qmckl_memcpy_H2D(context, coord_gpu, coord, size_max);
	...
	int size_0 = ctx->point.coord.size[0];
	if (transp == 'T') {
#pragma omp target is_device_ptr(a, coord_gpu)
		{
			for (int64_t i = 0; i < 3 * num; ++i) {
				a[i] = coord_gpu[i];
			}
...
#pragma omp target is_device_ptr(a, coord_gpu)
		{
			for (int64_t i = 0; i < num; ++i) {
				a[i] = coord_gpu[3 * i];
				a[i + size_0] = coord_gpu[3 * i + 1];
				a[i + 2 * size_0] = coord_gpu[3 * i + 2];
			}
		}
	}
