...
#pragma omp target teams distribute parallel for simd is_device_ptr(kf)
...
#pragma omp target is_device_ptr(b_vector, ee_distance_rescaled,               \
								 ...
								 factor_ee_deriv_e, kf, xk)
	{

		// TODO : Fix race condition & parallelize kernel
		for (int nw = 0; nw < walk_num; ++nw) {
			for (int j = 0; j < elec_num; ++j) {
				const double *dxj =
...
qmckl_exit_code_device qmckl_compute_jastrow_factor_en_deriv_e(
	const qmckl_context_device context, const int64_t walk_num,
	const int64_t elec_num, const int64_t nucl_num, const int64_t type_nucl_num,
	const int64_t *type_nucl_vector, const int64_t aord_num,
	const double *a_vector, const double *en_distance_rescaled,
...
qmckl_compute_tmp_c_device(const qmckl_context_device context,
	...
	gpu_dgemm('N', 'N', M, N, K, alpha, een_rescaled_e, LDA, een_rescaled_n,
			  LDB, beta, tmp_c, LDC);
...
#pragma omp declare target
	{
		...
		cublasStatus_t error = cublasDgemm(
			handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, K, &alpha, een_rescaled_e,
			LDA, een_rescaled_n, LDB, &beta, tmp_c, LDC);
			...
			for (int64_t i = 0; i < cord_num; ++i) {
				...
				for (int ii = 0; ii < M; ii++) {
					...
					for (int j = 0; j < N; j++) {
						...
						C[ii + LDC * j] = 0.;
						for (int k = 0; k < K; k++) {
							C[ii + LDC * j] += A[ii + k * LDA] * B[k + j * LDB];
						}
					}
				}
			}
