...
qmckl_vector qmckl_vector_alloc_device(qmckl_context_device context,
  ...
  result.data = qmckl_malloc_device(context, mem_info, device_id);
...
qmckl_exit_code qmckl_vector_free_device(qmckl_context_device context,
  ...
  rc = qmckl_free_device(context, vector->data, device_id);
  if (rc != QMCKL_SUCCESS) {
    return rc;
  }
...
qmckl_vector_of_double_device(const qmckl_context_device context,
  ...
  omp_target_memcpy(vector.data, target, vector.size * sizeof(double), 0,
...
qmckl_matrix qmckl_matrix_set_device(qmckl_matrix matrix,
                                            double value) {
...
qmckl_exit_code qmckl_tensor_free_device(qmckl_context_device context,
  ...
  rc = qmckl_free_device(context, tensor->data, device_id);
  if (rc != QMCKL_SUCCESS) {
    return rc;
  }
...
qmckl_tensor qmckl_tensor_set_device(qmckl_tensor tensor,
                                            double value) {
  // Recompute array size
  int prod_size = 1;
